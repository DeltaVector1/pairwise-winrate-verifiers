...
def _format_ab_prompt(prompt_a, completion_a, prompt_b, completion_b) -> str:
...
def load_environment(
    ...
    def _map_row(x: Dict[str, Any]) -> Dict[str, Any]:
        prompt = [
            ...
            {"role": "user", "content": x.get(question_key, "").strip()},
        ]
        info: Dict[str, Any] = {}
    ...
    def judge_fn(prompt_a, completion_a, prompt_b, completion_b, context: Dict[str, Any]) -> ABJudgeOutput:
        user_prompt = _format_ab_prompt(prompt_a, completion_a, prompt_b, completion_b)
        try:
